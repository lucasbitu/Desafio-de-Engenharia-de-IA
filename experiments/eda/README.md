...
```powershell
py -3 experiments\eda\01_dataset_overview\analyze.py
```

Para informar outro CSV:

```powershell
py -3 experiments\eda\01_dataset_overview\analyze.py --input "C:\caminho\tickets.csv"
```

## Execução da etapa 2

```powershell
py -3 experiments\eda\02_text_length\analyze.py
```

O script gera `experiments/eda/02_text_length/report.md` e `experiments/eda/02_text_length/extreme_examples.csv`. A execução deve ser feita pelo responsável pela análise; os resultados serão interpretados antes de avançar para duplicatas.

## Execução da etapa 3

```powershell
py -3 experiments\eda\03_duplicates\analyze.py
```

O script gera `experiments/eda/03_duplicates/report.md` e arquivos CSV de auditoria. A comparação normalizada altera somente espaços para detectar diferenças de formatação; ela não modifica o dataset original.
